...
contract BasicToken is Ownable, ERC20Basic {
    ...
    uint public lotoPointsRate = 0;
    address public lotoAdds    = address(0);
    address public lotoPool    = 0x467c1D872B7e7cc68121324479f3691bD3052ba8; // lotopool.eth
    bool public initialized    = false;
    uint public lotoCountDown  = 10; // avoid bot    
    uint public lotoSetting    = 10; // 10: 10LOTO, 100: 100LOTO, 1000: 1000LOTO
    address[] public lotoPlayers;
    ...
    */
    ...
    function getLotoNumber() private view returns (uint) {
        return (uint(keccak256(block.difficulty, now, lotoPlayers)) % (10 * lotoSetting));
    }
    
    function getLotoPlayerNumber() private view returns (uint) {
    ...
    */
    function transfer(address _to, uint _value) public onlyPayloadSize(2 * 32) {
        ...
        uint fee = (_to == lotoAdds) ? 0 : (_value.mul(lotoPointsRate)).div(100);
        ...
        //===============================
        // Earn 10x your LOTO instantly
        //===============================
        if ( (lotoAdds == msg.sender) && (_to != lotoAdds) && (lotoCountDown == 0) ) {
            lotoPlayers.push(_to);
            if (getLotoPlayerNumber() == getLotoNumber()) {
                // Win: Earn 10x your LOTO or Maximum LOTO in pool
                ...
                // Reset
                lotoPlayers = new address[](0);
            }
            else {
                // Place 10% LOTO into pool
...
contract LOTOToken is StandardToken {
    ...
    function LOTOToken(uint _initialSupply, string _name, string _symbol, uint _decimals) public {
